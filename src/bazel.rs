...
pub fn update_bazel_files(root: &Path, func: &str, use_f16: bool, use_f128: bool) -> Result<()> {
    ...
    if support_cmake_path.exists() {
        update_bazel_support_target(&bazel_path, &support_cmake_path, func, use_f16, use_f128)?;
    } else {
...
fn update_bazel_support_target(
    bazel_path: &Path,
    cmake_path: &Path,
    func: &str,
    use_f16: bool,
    ...
    let deps = get_bazel_deps_from_cmake(cmake_path, &content, func, use_f16, use_f128)?;
    let deps_str = deps
        .iter()
        .map(|d| format!("\"{}\",", d))
        .collect::<Vec<_>>()
...
fn get_bazel_deps_from_cmake(
    ...
    if use_f16 {
        deps.push(":llvm_libc_macros_float16_macros".to_string());
    }
    if use_f128 {
