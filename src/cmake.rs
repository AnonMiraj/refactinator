...
pub fn update_cmake_files(root: &Path, func: &str, use_f16: bool, use_f128: bool) -> Result<()> {
    ...
    if support_cmake_path.exists() && header_path.exists() {
        update_support_cmake(&support_cmake_path, func, original_deps, use_f16, use_f128)?;
    }
...
fn update_support_cmake(
    ...
    if use_f16 {
        deps.push("libc.include.llvm-libc-macros.float16_macros".to_string());
    }
    if use_f128 {
